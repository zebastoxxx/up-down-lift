...
export default function Machines() {
  ...
  const handleBulkDelete = async (selectedMachines: Machine[]) => {
    try {
      ...
      const {
        error
      } = await supabase.from('machines').delete().in('id', ids);
      if (error) throw error;
  ...
  const filteredMachines = machines.filter(machine => {
    const matchesSearch = machine.name.toLowerCase().includes(searchTerm.toLowerCase()) || machine.model?.toLowerCase().includes(searchTerm.toLowerCase()) || machine.brand?.toLowerCase().includes(searchTerm.toLowerCase()) || machine.location?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <div className="container mx-auto p-4 sm:p-6 space-y-6 max-w-7xl">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant={statusFilter === "operativo" ? "default" : "outline"} size="sm" onClick={() => setStatusFilter("operativo")}>
          Operativas ({statusCounts.operativo || 0})
        </Button>
        <Button variant={statusFilter === "mantenimiento" ? "default" : "outline"} size="sm" onClick={() => setStatusFilter("mantenimiento")}>
          Mantenimiento ({statusCounts.mantenimiento || 0})
        </Button>
        <Button variant={statusFilter === "fuera_de_servicio" ? "default" : "outline"} size="sm" onClick={() => setStatusFilter("fuera_de_servicio")}>
      ...
      <MachineForm isOpen={isFormOpen} onClose={() => {
      setIsFormOpen(false);
      setSelectedMachine(null);
    }} onMachineCreated={handleMachineCreated} machine={selectedMachine} />
    </div>;
}